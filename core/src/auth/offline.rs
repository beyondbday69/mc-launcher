...
pub fn offline_uuid(username: &str) -> String {
    ...
    // The digest is already a [u8; 16]; this is guaranteed to succeed.
...
mod tests {
    ...
    /// The vanilla offline UUID algorithm produces a specific value for
    /// the username "Notch" that has been cross-checked against the
    /// Minecraft server source. Locking it in via test prevents the
    /// algorithm from silently changing.
    #[test]
    fn offline_uuid_notch_matches_vanilla_algorithm() {
        // From public reference: OfflinePlayer:Notch -> UUID v3 with the
        // version + variant bits forced. Recompute with our implementation
        // and assert the well-known expected string.
        ...
        // Specific value: determinism check — calling twice yields the same.
        assert_eq!(offline_uuid("Notch"), u);
        // Different username -> different UUID.
    ...
    fn new_offline_account_validates_username() {
        ...
        assert!(new_offline_account("ab").is_err()); // too short
        assert!(new_offline_account("a".repeat(17).as_str()).is_err()); // too long
        assert!(new_offline_account("has space").is_err());
        assert!(new_offline_account("with-dash").is_err());
        // Username with valid char mix
    ...
    fn new_offline_account_has_no_tokens() {
        ...
        // UUID should be valid hex string of the expected shape.
