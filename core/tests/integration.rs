...
use mc_launcher_core::error::LauncherError;
use mc_launcher_core::launcher::jvm::JvmProfile;
use mc_launcher_core::metadata::inherits;
use mc_launcher_core::metadata::library;
use mc_launcher_core::security;
...
fn config_default_loads_from_missing_file() {
    ...
    let c = mc_launcher_core::config::LauncherConfig::load(&paths).unwrap();
    assert_eq!(c.version, mc_launcher_core::paths::CONFIG_VERSION);
...
fn config_save_and_reload_preserves_values() {
    let dir = tempdir();
    let paths = make_paths(&dir);
    let mut c = mc_launcher_core::config::LauncherConfig::default();
    c.default_ram_mb = 4096;
    c.default_jvm_profile = "performance".to_string();
    c.save(&paths).unwrap();
    let c2 = mc_launcher_core::config::LauncherConfig::load(&paths).unwrap();
...
fn instance_create_and_delete() {
    let dir = tempdir();
    let paths = make_paths(&dir);
    let store = mc_launcher_core::instances::InstanceStore::new(paths.clone());
...
"#;
    assert_eq!(mc_launcher_core::java::parse_java_version(s), 17);
}
...
"#;
    assert_eq!(mc_launcher_core::java::parse_java_version(s), 8);
}
...
"#;
    assert_eq!(mc_launcher_core::java::parse_java_version(s), 21);
}

#[test]
fn fabric_loader_kind_parses() {
    use mc_launcher_core::mods::LoaderKind;
...
fn pkce_pair_is_distinct() {
    let (a, b) = mc_launcher_core::auth::microsoft::pkce_pair();
    let (c, d) = mc_launcher_core::auth::microsoft::pkce_pair();
    assert_ne!(a, c);
    assert_ne!(b, d);
...
fn make_paths(root: &PathBuf) -> mc_launcher_core::paths::AppPaths {
    mc_launcher_core::paths::AppPaths {
        data_dir: root.clone(),
